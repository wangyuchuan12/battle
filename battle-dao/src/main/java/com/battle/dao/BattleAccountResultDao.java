...
public interface BattleAccountResultDao extends CrudRepository<BattleAccountResult, String>{
	...
	@Query(value="from com.battle.domain.BattleAccountResult bar where exists(select id from com.battle.domain.UserFriend uf where uf.userId=bar.userId and uf.friendUserId=:userId)   order by bar.level asc")
