...
public interface BattleRoomDao extends CrudRepository<BattleRoom, String>{
	...
	@Query("select br from com.battle.domain.BattleRoom br,com.battle.domain.BattlePeriodMember bpm where  br.id=bpm.roomId and bpm.userId=:userId and (bpm.status=1 or bpm.status=2) order by bpm.takepartAt desc")
